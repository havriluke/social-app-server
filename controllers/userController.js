...
class UserController {
    ...
    async check(req, res, next) {
        const user = await Users.findOne({attributes: ['id', 'nickname', 'role', 'banStatus', 'photo'], where: {id: req.user.id}})
        const token = generateJwt(user.id, user.nickname, user.role, user.banStatus, user.photo)
        return res.json({token})
    }
    ...
    async edit(req, res, next) {
        ...
        try {
            ...
            const {url} = await imgbbUploader({name: `u${userId}`, apiKey: process.env.IMGBB_API_KEY, base64string})
